...
// get dates for forecasted weather
var forecastedDates = function ([i]) {
  var currentDate = new Date();
  var forecastedDate =
    currentDate.getMonth() + 1 +
    "/" +
...
var forecastStats = function (forecastedDays, [i]) {
  ...
  futureDate.textContent = forecastedDates([i]);
  forecastUl[0].appendChild(futureDate);

  // Weather Icon
  var forecastimg = document.createElement("img");
  forecastimg.src =
    "http://openweathermap.org/img/wn/" +
    forecastedDays[i][0].weather[0].icon +
    "@2x.png";
  ...
  forecastHumidityLi.textContent =
    "Humidity: " + forecastedDays[i][0].humidity + "%";
  forecastUl[0].appendChild(forecastHumidityLi);

  // Wind Speed
  var forecastWindSpeedLi = document.createElement("li");
  forecastWindSpeedLi.textContent =
    "Wind: " + forecastedDays[i][0].wind_speed + " MPH";
